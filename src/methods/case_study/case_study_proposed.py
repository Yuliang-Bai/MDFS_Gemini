...
class MDFSClassifier(BaseMethod):
    ...
    def fit(self, X: Dict[str, np.ndarray], y: np.ndarray,
        ...
        self.gating_net.train(); self.compress_net.train(); self.predict_net.train()

        if verbose:
            print(f"\n[{self.name}] Start Training (Epochs: {self.epochs})...")
            # 【修改点】移除了 Recall，纯净打印 Loss
            print(f"{'Epoch':<6} | {'Tot_Loss':<10} | {'Cls_Loss':<10} | {'Rec_Loss':<10} | {'Ent_Loss':<10} | {'Sp_Loss':<10}")
            print("-" * 65)
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            # 【修改点】无论是否有 true_features，都强制在 verbose 下打印 debug Loss
            if verbose and (epoch % 10 == 0 or epoch == self.epochs):
                print(f"{epoch:<6} | {total_loss.item():<10.4f} | {loss_c.item():<10.4f} | {recon_loss.item():<10.4f} | {loss_ent.item():<10.4f} | {loss_sp.item():<10.4f}")
