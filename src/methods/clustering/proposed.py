...
class MDFSClustering(BaseMethod):
    """MDFS 聚类专用版"""
    def __init__(self, name="MDFS_Clu", latent_dim=10, temperature=1.0, hidden_ratio=0.5, **kwargs):
    ...
    def _get_gating_weights(self, reps):
        scores = []
        for r in reps:
            var_score = np.sum(np.var(r, axis=0))
            scores.append(var_score)
        scores = np.array(scores)
        scores = scores / (np.max(scores) + self.epsilon)
    ...
    def fit(self, X: Dict[str, np.ndarray], y: Optional[np.ndarray] = None) -> FitResult:
        ...
        alpha = self._get_gating_weights(reps)
