    ...
    def fit(self, X: Dict[str, np.ndarray], y: Optional[np.ndarray] = None,
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            if true_features and (epoch % 20 == 0 or epoch == self.epochs):
                ...
                if verbose:
                    print(f"{epoch:<6} | {total_loss.item():<10.4f} | {recon_loss.item():<10.4f} | {loss_ent.item():<10.4f} | {loss_sp.item():<10.4f} | {metrics['recall_total']:<8.4f}")

                self.encoders.train(); self.compress_net.train(); self.gating_net.train()
